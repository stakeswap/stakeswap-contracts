...
contract Router is IRouter {
    ...
    constructor(address _factory, address payable _WETH) {
        factory = _factory;
        WETH = _WETH;
        pairInitCodeHash = IFactory(factory).pairInitCodeHash();
    }
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = UniswapV2Library.getReserves(pairInitCodeHash, factory, tokenA, tokenB);
    ...
    function addLiquidity(
        ...
        address pair = UniswapV2Library.pairFor(pairInitCodeHash, factory, tokenA, tokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = UniswapV2Library.pairFor(pairInitCodeHash, factory, token, WETH);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? UniswapV2Library.pairFor(pairInitCodeHash, factory, output, path[i + 2]) : _to;
            IPair(UniswapV2Library.pairFor(pairInitCodeHash, factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = UniswapV2Library.getAmountsOut(pairInitCodeHash, factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            UniswapV2Library.pairFor(pairInitCodeHash, factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = UniswapV2Library.getAmountsIn(pairInitCodeHash, factory, amountOut, path);
    ...
    function swapExactETHForTokens(
        ...
        amounts = UniswapV2Library.getAmountsOut(pairInitCodeHash, factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        WETHInterface(WETH).deposit{ value: amounts[0] }();
        assert(WETHInterface(WETH).transfer(UniswapV2Library.pairFor(pairInitCodeHash, factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = UniswapV2Library.sortTokens(input, output);
            IPair pair = IPair(UniswapV2Library.pairFor(pairInitCodeHash, factory, input, output));
            uint amountInput;
            uint amountOutput;
            {
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(WETHInterface(WETH).transfer(UniswapV2Library.pairFor(pairInitCodeHash, factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    function getAmountsOut(uint amountIn, address[] memory path) public view virtual override returns (uint[] memory amounts) {
        return UniswapV2Library.getAmountsOut(pairInitCodeHash, factory, amountIn, path);
    }

    function getAmountsIn(uint amountOut, address[] memory path) public view virtual override returns (uint[] memory amounts) {
        return UniswapV2Library.getAmountsIn(pairInitCodeHash, factory, amountOut, path);
    }
}
