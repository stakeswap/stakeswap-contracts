...
contract FraxAdapter is BaseAdapter {
    ...
    function getTokens() public view override returns (address token0, address token1, address token2) {
        token0 = address(frxETH());
        token1 = address(sfrxETH());
        token2 = address(frxETHMinter());
    }
    ...
    function deposit() public payable override returns (uint256) {
        ...
        // ETH -> sfrxETH with msg.sender as receiver
        uint256 sfrxAmount = frxETHMinter().submitAndDeposit{value: msg.value}(msg.sender);
        return sfrxAmount;
    }
