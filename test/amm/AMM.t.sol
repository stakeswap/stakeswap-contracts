...
contract AMMTest is Constants, Test, MinorError {
    ...
    function setUp() public {
        ...
        vm.startPrank(user0);
        DAI().approve(address(pair_WETH_DAI), type(uint256).max);
        DAI().approve(address(pair_WETH_USDC), type(uint256).max);
        DAI().approve(address(pair_DAI_USDC), type(uint256).max);
        WETH().approve(address(pair_WETH_DAI), type(uint256).max);
        WETH().approve(address(pair_WETH_USDC), type(uint256).max);
        WETH().approve(address(pair_DAI_USDC), type(uint256).max);
        USDC().approve(address(pair_WETH_DAI), type(uint256).max);
        USDC().approve(address(pair_WETH_USDC), type(uint256).max);
        USDC().approve(address(pair_DAI_USDC), type(uint256).max);
        vm.stopPrank();
        vm.startPrank(user1);
        DAI().approve(address(pair_WETH_DAI), type(uint256).max);
        DAI().approve(address(pair_WETH_USDC), type(uint256).max);
    ...
    function test_pair_WETH_DAI() public {
        ...
        vm.startPrank(user0);
        uint256 lp0 = pair.mint(user0);
        vm.stopPrank();
        vm.startPrank(user1);
        uint256 lp1 = pair.mint(user1);
        vm.stopPrank();
